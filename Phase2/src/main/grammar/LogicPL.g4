...
statement returns [Statement statementRet]:
    s1 = assignSmt {$statementRet = $s1.assignStmtRet;}
    | (s2 = predicate SEMICOLON {$statementRet = $s2.predicateRet;})
    | s3 = implication {$statementRet = $s3.implicationRet;}
    | s4 = returnSmt {$statementRet = $s4.returnRet;}
    | s5 = printSmt {$statementRet = $s5.PrintStmtRet;}
    | s6 = forLoop {$statementRet = $s6.forRet;}
    | s7 = localVarDeclaration {$statementRet = $s7.localvarDecRet;}
    ;
...
variable returns [Variable v]:
    i = identifier {$v = $i.identifierRet;}
    | name = identifier LBRACKET exp = expression RBRACKET
    {$v = new ArrayAccess($name.identifierRet.getName(), $exp.e); $v.setLine($name.identifierRet.getLine()); }
    ;
...
varDeclaration returns [VarDecStmt varDecRet]:
    ...
    {$varDecRet = new VarDecStmt($var_iden.identifierRet, $t.typeRet);
    $varDecRet.setLine($var_iden.identifierRet.getLine());}
    (ASSIGN e = expression {$varDecRet.setInitialExpression($e.e);} )? SEMICOLON
    ;

arrayDeclaration returns [ArrayDecStmt arrayDecRet]:
    t = type LBRACKET INT_NUMBER RBRACKET var_iden = identifier
    {$arrayDecRet = new ArrayDecStmt($var_iden.identifierRet, $t.typeRet, $INT_NUMBER.int);
    $arrayDecRet.setLine($var_iden.identifierRet.getLine());}
...
arrayList returns [ArrayList<Expression> values]:
    ...
    LBRACKET (v = value {$values.add($v.valueRet);}
    | id = identifier {$values.add($id.identifierRet);}) (COMMA (v2 = value {$values.add($v2.valueRet);}
    | id2 = identifier {$values.add($id.identifierRet);}))* RBRACKET
    ;

printSmt returns [PrintStmt PrintStmtRet]:
    p = PRINT LPAR arg = printExpr RPAR SEMICOLON
    {$PrintStmtRet = new PrintStmt($arg.printExprRet);
...
queryType1 returns [QueryExpression query1Ret]:
    ...
    {$query1Ret = new QueryExpression($id.predicateIdentifierRet);
    $query1Ret.setLine($line.getLine()); $query1Ret.setVar($var.v);}
    ;

queryType2 returns [QueryExpression query2Ret]:
    LBRACKET id = predicateIdentifier LPAR line = QUARYMARK RPAR RBRACKET
    {$query2Ret = new QueryExpression($id.predicateIdentifierRet);
    $query2Ret.setLine($line.getLine()); }
    ;

returnSmt returns [ReturnStmt returnRet]:
    RETURN (v = value {$returnRet = new ReturnStmt($v.valueRet);}
    | iden = identifier {$returnRet = new ReturnStmt($iden.identifierRet);})? SEMICOLON 
    {if($returnRet == null){$returnRet = new ReturnStmt(null);}}
...
forLoop returns [ForloopStmt forRet]:
    ...
    {$forRet = new ForloopStmt($iterator.identifierRet, $arrayName.identifierRet, bodyStmts);
    $forRet.setLine($line.getLine());}
    ;

predicate returns [PredicateStmt predicateRet]:
    id = predicateIdentifier LPAR v = variable RPAR
    {$predicateRet = new PredicateStmt($id.predicateIdentifierRet, $v.v);
    $predicateRet.setLine($id.predicateIdentifierRet.getLine());}
    ;
...
expression returns [Expression e]:
    ...
    {if($r.e != null) {$e = new BinaryExpression($l.e, $r.e.getRight(), $r.e.getBinaryOperator());
    $e.setLine($r.e.getLine());} else {$e = $l.e;}}
    ;

expression2 returns [BinaryExpression e] locals [BinaryExpression ee]:
    OR l = andExpr r = expression2
    {if($r.e != null) {$ee = new BinaryExpression($l.e, $r.e.getRight(), $r.e.getBinaryOperator());
    $ee.setLine($r.e.getLine()); $e = new BinaryExpression(null, $ee, BinaryOperator.or);} else{$e = new BinaryExpression(null, $l.e, BinaryOperator.or);}}
...
andExpr2 returns [BinaryExpression e] locals [BinaryExpression ee]:
    ...
    {if($r.e != null) {$ee = new BinaryExpression($l.e, $r.e.getRight(), $r.e.getBinaryOperator());
    $ee.setLine($r.e.getLine());
    $e = new BinaryExpression(null, $ee, BinaryOperator.and);} else{$e = new BinaryExpression(null, $l.e, BinaryOperator.and);}}
...
eqExpr2 returns [BinaryExpression e] locals [BinaryOperator opt, BinaryExpression ee]:
    ...
    {if($r.e != null) {$ee = new BinaryExpression($l.e, $r.e.getRight(), $r.e.getBinaryOperator());
    $ee.setLine($r.e.getLine()); $e = new BinaryExpression(null, $ee, $opt);} else{$e = new BinaryExpression(null, $l.e, $opt);}}
...
other returns [Expression e]:
    LPAR expr = expression RPAR {$e = $expr.e;}
    | var = variable {$e = $var.v;}
    | val = value {$e = $val.valueRet;}
    | q1 = queryType1 {$e = $q1.query1Ret;}
    | funcCall = functionCall {$e = $funcCall.fCallRet;}
    ;
...
predicateIdentifier returns [Identifier predicateIdentifierRet]:
    predicate_idnfr = PREDICATE_IDENTIFIER {$predicateIdentifierRet = new Identifier($predicate_idnfr.text);
    $predicateIdentifierRet.setLine($predicate_idnfr.getLine());}
    ;
